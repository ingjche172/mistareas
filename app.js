...
function startEmployeeLogin(empId) {
  ...
  document.getElementById('pin-modal-sub').textContent   = 'Ingresa tu PIN';
  pinBuffer = ''; updatePinDots();
  document.getElementById('pin-error').classList.add('hidden');
...
// ===========================
function pinKey(d) {
  const maxLen = pinTarget === 'admin' || pinTarget === 'change-admin' ? 7 : 4;
  if(pinBuffer.length >= maxLen) return;
  pinBuffer += d; updatePinDots();
  // Auto-submit solo para empleados al llegar a 4 dígitos
  if(pinTarget !== 'admin' && pinTarget !== 'change-admin' && pinBuffer.length === 4) setTimeout(pinSubmit, 200);
}
function pinClear() {
...
function updatePinDots() {
  const isAdmin = pinTarget === 'admin' || pinTarget === 'change-admin';
  const maxDots = isAdmin ? 7 : 4;
  // Mostrar u ocultar dots dinámicamente
  ...
  if(container) {
    ...
    for(let i=1; i<=maxDots; i++){
      ...
      span.className = 'dot';
      span.id = 'd'+i;
      if(i <= pinBuffer.length) span.classList.add('filled');
...
async function saveEmployee() {
  ...
  if(!/^\d{4}$/.test(pin)){ showToast('⚠️ El PIN debe ser 4 dígitos'); return; }
  const id = editingEmpId || Date.now().toString();
  const emp = { id, name, pin, color: selectedColor };
  try {
