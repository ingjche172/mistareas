...
function pinSubmit() {
  ...
  if(!correct){
    for(let i=1;i<=4;i++){ const d=document.getElementById('d'+i); d.classList.add('error'); d.classList.remove('filled'); }
...
function enterAdmin() {
  ...
  showAdminTab('new');
  populateAssignSelect(); populateFilterSelect();
  renderAdminTasks(); renderEmployeeList(); updateAdminBadge();
}

function showAdminTab(tab) {
  ['new','tasks','team'].forEach(t => {
