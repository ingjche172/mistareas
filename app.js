...
function openTaskDetail(taskId) {
  ...
  // Foto de evidencia del empleado
  const donePhotoBox = document.getElementById('detail-done-photo-box');
  const donePhotoImg = document.getElementById('detail-done-photo');
  if(task.donePhoto){ donePhotoBox.classList.remove('hidden'); donePhotoImg.src=task.donePhoto; } else { donePhotoBox.classList.add('hidden'); }
...
function openDoneComment(taskId) {
  ...
  // Reset foto de evidencia
  donePhotoData = null;
  document.getElementById('done-photo-preview').classList.add('hidden');
  document.getElementById('done-photo-placeholder').classList.remove('hidden');
  document.getElementById('done-photo-remove-btn').classList.add('hidden');
  document.getElementById('done-photo-input').value = '';
  closeModal('modal-task-detail'); openModal('modal-done');
}

function handleDonePhoto(e) {
  const file = e.target.files[0]; if(!file) return;
  const reader = new FileReader();
  reader.onload = ev => {
    const img = new Image();
    img.onload = () => {
      ...
      canvas.width = img.width*ratio; canvas.height = img.height*ratio;
      canvas.getContext('2d').drawImage(img, 0, 0, canvas.width, canvas.height);
      donePhotoData = canvas.toDataURL('image/jpeg', 0.7);
      document.getElementById('done-photo-preview').src = donePhotoData;
      document.getElementById('done-photo-preview').classList.remove('hidden');
      document.getElementById('done-photo-placeholder').classList.add('hidden');
      document.getElementById('done-photo-remove-btn').classList.remove('hidden');
    };
    img.src = ev.target.result;
  };
  reader.readAsDataURL(file);
...
async function submitDone() {
  ...
  try {
    await db.collection('tasks').doc(doneTaskId).update({
      status: 'done',
      doneAt: new Date().toISOString(),
      doneComment: comment,
      donePhoto: donePhotoData || null
    });
    donePhotoData = null;
